...
let http_handler = (req, res) => {
    ...
    if(RegExp(/^\/parameter\/[0-9][0-9]\/[0-9][0-9]/).test(url.parse(req.url).pathname)) {
        res.writeHead(200, {'Content-Type':'text/html; charset=utf-8'});
        var p = url.parse(req.url, true);

        if((x = +p.pathname.split('/')[2]) && (y = +p.pathname.split('/')[3])) {
    ...
    if (url.parse(req.url).pathname === '/files') {
        res.setHeader('X-static-files-count', fs.readdirSync('./node_modules').length);
        res.end();
    }
